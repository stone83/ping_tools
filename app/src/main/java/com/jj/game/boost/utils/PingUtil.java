...
//        }
        String pingString;
        if (timeout != 0) {
            pingString = ping(createSimplePingCommand(count, timeout, url));
        } else {
            pingString = ping(createSimplePingCommand(count, url));
        }
        LogUtil.i("pingString -> " + pingString);
        if (null != pingString) {
            try {
                String tempInfo = pingString.substring(pingString.indexOf("min/avg/max/mdev") + 19);
                String[] temps = tempInfo.split("/");
     ...
     */
    @SuppressWarnings("unused")
    public static int getAvgRTT(String url, int count) {
        return getAvgRTT(url, count, 0);
    }

    public static double getAvgRTTValue(String pingString) {
//        String domain = getDomain(url);
//        if (null == domain) {
//            return -1;
//        }
        if (pingString != null) {
            try {
                String tempInfo = pingString.substring(pingString.indexOf("min/avg/max/mdev") + 19);
                return BigDecimal.valueOf(Double.valueOf(tempInfo.split("/")[1]))
     ...
     */
    @SuppressWarnings("unused")
    public static double getPacketLossDouble(String url, int count, int timeout) {
        String packetLossInfo = getPacketLoss(url, count, timeout);
        if (null != packetLossInfo) {
            try {
                return Double.valueOf(packetLossInfo.replace("%", ""));
            } catch (Exception e) {
                e.printStackTrace();
            }
        }
     ...
     */
    @SuppressWarnings("unused")
    public static double getPacketLossDouble(String url) {
